...
const AddUpdateModal = ({ open, handleModal, activity, isadd }) => {
  ...
  function handleChange(name, value) {
    ...
    if (isadd) {
      if (name === "imageFile") {
        //last Name validation
        if (!value.name) {
          errors.imageFile = "Icon is required";
        } else {
          errors.imageFile = "";
        }
      }
  ...
  const AddType = () => {
    ...
    formData.append("image", inputValues.imageFile);
    axios({
      method: "post",
  ...
  const UpdateMenuCatagory = () => {
    ...
    axios({
      method: "put",
      url: `/admin/banner/update/${activity.id}`,
      ...
      .then((response) => {
        console.log("menuItmeStaus", response);
        handleModal();
        navigate(`${process.env.REACT_APP_FOLDER}/banners`);
        // setModal(false)
    ...
    >
      ...
      >
        <h5 className="modal-title">
          {isadd ? 'Add Banner' : `Update Banner #${activity.id}`}
        </h5>
      </ModalHeader>
      <ModalBody className="flex-grow-1">
        <div className="mb-1" style={{ padding: "10px" }}>
          ...
          <div className="flex justify-end items-end">
            ...
            >
              Image <span><i>( Recommend Size : 800 x 400 )</i></span>
            </Label>
            <div>
              <img
