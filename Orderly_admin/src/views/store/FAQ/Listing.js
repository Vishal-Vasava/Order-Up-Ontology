...
const Listing = () => {
    ...
    if (value.length) {
      updatedData = activities.filter((item) => {
        const startsWith =
          ...
          item.answer.toLowerCase().startsWith(value.toLowerCase()) ||
        ...
        const includes =
          ...
          item.answer.toString().toLowerCase().includes(value.toLowerCase()) ||
  ...
  const getRecord = () => {
    axios
      .get("/admin/faq", { headers: { Authorization: `Bearer ${token}` } })
      .then((response) => {
    ...
    <div>
      ...
      <Card>
        <CardHeader>
          ...
          >
            <Plus size={15} />
            <span className="align-middle ms-50">Create New FAQs</span>
          </Button>
        </CardHeader>
