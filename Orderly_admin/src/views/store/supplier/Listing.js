...
import { ActivityListColumns, SupplierTypeListColumns } from "../../data";
import {
  ChevronDown,
  Share,
  Printer,
    ...
    if (value.length) {
      updatedData = activities.filter((item) => {
        const startsWith =
          ...
          (item.status == true ? "Active" : "Inactvie").toLowerCase().startsWith(value.toLowerCase());

        const includes =
          item.id.toString().toLowerCase().includes(value.toLowerCase()) ||
          item.name.toLowerCase().includes(value.toLowerCase()) ||
        (item.status == true ? "Active" : "Inactvie").toLowerCase().includes(value.toLowerCase());
    ...
    <div>
      ...
      <Card>
        <CardHeader>
          ...
          >
            <Plus size={15} />
            <span className="align-middle ms-50">Create New Store</span>
          </Button>
        </CardHeader>
      </Card>
      <Card>
        ...
        <div className="react-dataTable react-dataTable-selectable-rows">
          <DataTable
            ...
            pagination
            // selectableRows
            columns={SupplierTypeListColumns}
            paginationPerPage={7}
