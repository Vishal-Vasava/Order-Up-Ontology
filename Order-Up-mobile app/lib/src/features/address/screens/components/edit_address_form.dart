...
class EditAddressForm extends StatefulWidget {
  const EditAddressForm({super.key, required this.addressId});
...
class _EditAddressFormState extends State<EditAddressForm> {
  ...
  late TextEditingController houseFlatNoController;
  ...
  void initState() {
    ...
    final index = context
        .read<AddressCubit>()
        .addressList
        .indexWhere((element) => element.id! == widget.addressId);
    if (index > -1) {
      model = context.read<AddressCubit>().addressList[index];
      ...
      zipController = TextEditingController(text: model.zipCode!);
      emailController = TextEditingController(text: model.email!);
      mobileController = TextEditingController(text: model.phone!);
      houseFlatNoController =
          TextEditingController(text: model.houseNo?.toString());
      addressController = TextEditingController(text: model.address!);
      streetNoController = TextEditingController(text: model.street!);
      flatNoController = TextEditingController(text: model.houseNo?.toString());
      cityController = TextEditingController(text: model.city);
      stateController = TextEditingController(text: model.state);
      countryController = TextEditingController(text: model.country);
      selectedCountry.value =
          countryController.text.toLowerCase().contains('india')
              ? 'India'
              : 'US';
      isHomeAddress.value = model.type!.toLowerCase().contains('home');
      markAsDefault.value = model.isDefault!;
      ...
      houseFlatNoController = TextEditingController();
  ...
  Future<void> _callAPIForPincode({String countryData = ''}) async {
    await Future.delayed(const Duration(milliseconds: 800));
    late PostalCode? postalCode;
    try {
      final country = model.country ?? 'IN';
      final url = Endpoints.worldPostalLocationApi(
        pinCode: zipController.text.trim(),
        countryCode: countryData.isNotEmpty
            ? countryData
            : country.isEmpty
                ? 'IN'
                : country,
                ...
                TextFormField(
                  ...
                  onChanged: (value) {
                    if (value.length >= 4) {
                      _callAPIForPincode(
                        countryData:
                            selectedCountry.value == 'India' ? 'IN' : 'US',
                      );
                    }
                  },
                  onEditingComplete: () {
                ...
                BlocConsumer<AddressCubit, AddressState>(
                  listenWhen: (_, newState) {
                    return newState is AddressUpdateFailedState ||
                        newState is AddressUpdateSuccessState;
                  },
                  ...
                  buildWhen: (_, newState) {
                    return newState is AddressUpdateFailedState ||
                        newState is AddressUpdateSuccessState ||
                        newState is AddressUpdateLoadingState;
                  },
                  builder: (context, state) {
                    return AppButton(
                      isLoading: state is AddressUpdateLoadingState,
                      buttonText: 'Submit',
                      onPressed: () async {
                        if (formKey.currentState?.validate() ?? false) {
                          final model = AddressModel(
                            ...
                            city: postResultList.isNotEmpty
                                ? postResultList[0].district
                                : cityController.text.trim(),
                            state: postResultList.isNotEmpty
                                ? postResultList[0].state
                                : stateController.text.trim(),
                            country: postResultList.isNotEmpty
                                ? postResultList[0].country
                                : countryController.text.trim(),
                            streetNo: streetNoController.text.trim(),
                            flatNo: flatNoController.text.trim(),
                            addLatitude: postResultList.isNotEmpty
                                ? postResultList[0].latitude
                                : latitude,
                            addLongitude: postResultList.isNotEmpty
                          ...
                          await context.read<AddressCubit>().updateAddress(
                              addressId: widget.addressId, model: model);
                        }
                      },
                    );
                  },
